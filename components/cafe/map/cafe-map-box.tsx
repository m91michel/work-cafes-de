...
import Map, { Marker, Popup, NavigationControl, MarkerEvent, useMap } from 'react-map-gl/mapbox'
...
export function CafeMapBox({ cafes, className }: Props) {
  ...
  const handleMarkerClick = useCallback((e: MarkerEvent<MouseEvent>, cafe: Cafe) => {
        ...
        {popupInfo && (
          ...
          >
            <Card className="p-3">
              ...
              <p className="text-sm text-muted-foreground mt-2">{popupInfo.address}</p>
            </Card>
          </Popup>
        )}
      </Map>
